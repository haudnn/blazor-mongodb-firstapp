...
public class TodoData
{
    ...
    public async Task CreateTodoMigration(TodoModel todo)
    {
        var database = _client.GetDatabase("todo-list");
        var todoListCollection = database.GetCollection<TodoListModel>("todo-list");
        var todoCollection = database.GetCollection<TodoModel>("todo");
        // Create TodoList
        // var filter = Builders<TodoListModel>.Filter.Eq("dateCreated", todo.date);
        var todoList = await todoListCollection.Find(x => x.dateCreated == todo.date).FirstOrDefaultAsync();
        if (todoList?.Any() ?? false)
        {
            var todoUpdate = new TodoModel { todo, todoList = todoList._id };
            await todoCollection.InsertOneAsync(todoUpdate);
            // not null
        }
        // null
